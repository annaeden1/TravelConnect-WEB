...
const Comment = ({ comment, onDelete }: CommentProps) => {
    ...
    const isCreaterOfComment = comment.userCreator.username === getCurrentUser()?.username;
        ...
        <Box sx={{ border: '1px solid #ccc', borderRadius: 2, p: 2, mb: 2, backgroundColor: 'background.paper' }}>
            ...
            <Box sx={{ p: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
                <Avatar sx={{ bgcolor: 'primary.main' }}>
                    {comment.userCreator.profileImage ? (
                        <img src={comment.userCreator.profileImage} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="profile" />
                    ) : (
            ...
            {isCreaterOfComment && (
                <Box sx={{ display: 'flex', gap: 1, mt: 2, px: 2 }}>
                    ...
                    <Button onClick={() => onDelete(comment._id)} variant="outlined" color="error" size="small">
                        <Delete />
                    </Button>
                </Box>
